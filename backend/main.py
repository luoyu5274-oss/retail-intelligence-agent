...
5. 多品牌/多年份数据对比时，必须用Markdown表格呈现。表格必须同时包含数值列和增速/比率列，以便前端渲染组合图（柱状图+折线图）。格式：行为财年，列为各品牌的数值+增速。例如：

| 财年 | Nike 营收 (M) | Adidas 营收 (M) | Lululemon 营收 (M) | Nike 增速 (%) | Adidas 增速 (%) | Lululemon 增速 (%) |
|------|-------------|----------------|-------------------|--------------|---------------|-------------------|
| FY2022 | 46710 | 21234 | 8110 | — | — | — |
| FY2023 | 51217 | 21427 | 9619 | 9.6 | 0.9 | 18.6 |
| FY2024 | 51362 | 23683 | 10588 | 0.3 | 10.5 | 10.1 |
...
def health_check():
    ...
    try:
        steps.append("start")
        indexed = vector_store.is_indexed()
        steps.append(f"indexed={indexed}")
        results = vector_store.query("test")
        steps.append(f"query_ok({len(results)})")
        from config import DEEPSEEK_API_KEY
        steps.append(f"key={'set' if DEEPSEEK_API_KEY else 'missing'}")
